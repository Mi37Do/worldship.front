<template>
  <div
    ...
    <span class="h-8 my-auto flex items-center truncate">$ {{ route.name === 'warehouse-invoices' ?
      numberFormat(item.wh_pk.total_price) : numberFormat(item.bfm_pk.total_price) }} </span>
...
const openDetails = async () => {
  ...
  await useInvoices.getInvoices(route.name === 'warehouse-invoices' ? 'wrhs' : 'bfm', null, props.item.id)
