<template>
  <input type="checkbox" id="userDeleteModal" :checked="useWidget.buyForMeInbox" class="modal-toggle" />
  <div class="modal" role="dialog">
    <div class="modal-box flex flex-col min-h-80 gap-2 items-center justify-center p-4">
      ...
      <form v-else @submit.prevent="buyForMeInbox(item.id)"
...
watch(() => useWidget.deleteModal, () => {
  if (useWidget.buyForMeInbox) return
  else {
    loadingReturn.value = false
  }
}, { deep: true })
