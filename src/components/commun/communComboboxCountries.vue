    ...
    <Combobox v-model="selected">
      <div class="relative">
        <div class="relative w-full flex items-center">
          <img :src="selected.image" class="w-7 absolute left-2" alt="">
          <ComboboxInput autocomplete="off" :required="required"
            class="w-full pixa-input placeholder:capitalize ring-inset focus:ring-0 px-4 pl-10"
        ...
        <TransitionRoot leave="transition ease-in duration-100" leaveFrom="opacity-100" leaveTo="opacity-0"
          ...
          <ComboboxOptions :class="[top ? '' : 'mt-1']"
            ...
            <ComboboxOption v-for="item in filteredList" as="template" :key="item.id" @click="selectItem(item.id)"
              ...
              <li class="relative cursor-default select-none py-2 pl-10 pr-4 w-full flex gap-3 items-center" :class="{
                'bg-primary/20 text-primary fill-primary': active,
                'text-gray-900': !active,
              }">
                <img :src="item.image" class="w-6" alt="">
