<template>
  <div class="w-full h-full flex flex-col overflow-hidden">
    <div :class="[useWidget.userLanguage === 'ar' ? 'text-right  pl-7' : ' pr-7']"
      class="w-full h-12 rounded-lg border border-slate-200 px-4 hidden lg:grid grid-cols-8 gap-2 uppercase font-bold text-primary bg-white">
    ...
    <div class="w-full flex-1 overflow-auto flex flex-col">
      <div class="w-full h-fit flex flex-col gap-2 pt-2">
        <itemData v-for="item in useBook.filtredAdresses.filter(it => it.type === 'from')" :key="item" :item="item" />
      </div>
    </div>
  </div>
</template>
