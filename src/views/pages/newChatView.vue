<template>
  ...
  <div v-else class="drawer drawer-end lg:drawer-open bg-[#f6f9ff] w-full h-screen overflow-hidden">
    <input id="my-drawer-2" :checked="openChatSide" type="checkbox" class="drawer-toggle" />
    <div :class="[
      'drawer-content flex flex-col items-center justify-center  w-full h-full overflow-hidden']">
      ...
      <div class="w-full h-full  flex flex-col overflow-hidden">
        ...
        <div
          ...
          <div class="flex flex-1 gap-3 items-center">
            <router-link :to="{ name: 'support' }" class="btn btn-sm pixa-btn w-10 p-0 pixa-btn-nofloat">
              <angle-right-icon class="w-5 h-5 rotate-180" />
            </router-link>
            <span class="pixa-title truncate">{{ useSupport.focusedTicket.name }}</span>
          </div>
          ...
          <div class="flex gap-1.5 items-center">
            ...
            <button @click="openChatSide = true" class="btn btn-sm pixa-btn w-10 p-0 pixa-btn-nofloat">
        ...
        <div class="w-full p-4 flex flex-col gap-4  border-t border-slate-200">
          <div v-if="imagePreview" class="w-20 h-20 bg-slate-200 rounded-md overflow-hidden relative">
            ...
            }" class="btn btn-sm pixa-btn w-10 absolute top-1 right-1 btn-ghost">
    ...
    <div class="drawer-side">
      <label @click="openChatSide = false" aria-label="close sidebar" class="drawer-overlay"></label>
      <div class="menu text-base-content h-full w-full flex flex-col gap-2 bg-white">

        <div class="w-full h-full items-center overflow-hidden">
          <div class="w-full h-full flex flex-col overflow-auto p-4">

            <div class="w-full h-fit flex flex-col py-4 gap-4 items-center relative">

              <button @click="openChatSide = false"
                class="btn btn-sm pixa-btn w-10 p-0 absolute right-4 top-4 pixa-btn-nofloat">
